...
def make_data(patch_size=9):
    ...
    # load images
    # filepaths = glob.glob("D:/Berkeley/Spring/stat-214/image_data.zip")
    zip_path = 'image_data.zip'
    with zipfile.ZipFile(zip_path, "r") as zip_file:
        npz_files = [f for f in zip_file.namelist() if f.startswith("image_data/") and f.endswith(".npz")]
        images_long = []
        for fp in npz_files:
            with zip_file.open(fp) as npz_file:
